...
public class EpidemicAnalysisController extends BaseController {
	...
	public AjaxResult get_death_rate(@PathVariable String illnessName) {
		...
		// ArrayList<EpidemicCase> list =
		// epidemicAnalysisService.getCaseListByIllnessName(illnessName);
		// int death_number = 0;
		...
		return AjaxResult.success("success", epidemicAnalysisService.getDeathRate(illnessName));
	}
	...
	public AjaxResult get_diff_time(@PathVariable String illnessName) {
		// ArrayList<EpidemicCase> list =
		// epidemicAnalysisService.getCaseListByIllnessName(illnessName);
		// ArrayList<DiffTimeVo> finalList = new ArrayList<>();
		// // <days,peopleCount>
		...
		// int betweenDays = (int) ((timeInMillis2 - timeInMillis1) / (1000L * 3600L *
		// 24L));
		...
		// DiffTimeVo cureTimeVo = new DiffTimeVo();
		// cureTimeVo.setCureTime(map_.getKey());
		// cureTimeVo.setCuredCount(map_.getValue());
		// finalList.add(cureTimeVo);
		// }
		return AjaxResult.success("success", epidemicAnalysisService.getDiffTime(illnessName));
	}
	...
	public AjaxResult get_patient_info(@PathVariable String illnessName) {
		// ArrayList<EpidemicPatient> list =
		// epidemicPatientService.getPatientListByIllnessName(illnessName);
		...
		// }
		return AjaxResult.success("success", epidemicAnalysisService.getPatientInfo(illnessName));
	}
	...
	public AjaxResult get_region_info(@PathVariable String illnessName) {
		// ArrayList<EpidemicPatient> patientList =
		// epidemicPatientService.getPatientListByIllnessName(illnessName);
		...
		return AjaxResult.success("success", epidemicAnalysisService.getRegionInfo(illnessName));
	}

	@GetMapping("get_drug/{illnessName}")
	public AjaxResult get_drug(@PathVariable String illnessName) {
		return AjaxResult.success("success", epidemicAnalysisService.getDrug(illnessName));
	}

	@GetMapping("get_pat_history/{patientName}")
	public AjaxResult get_pat_history(@PathVariable String patientName) {
		return AjaxResult.success("success", epidemicAnalysisService.getPatHistory(patientName));
	}

	@GetMapping("get_seq_comp/{illnessName}")
