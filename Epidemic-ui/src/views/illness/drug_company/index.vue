    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['illness:drug_company:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['illness:drug_company:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['illness:drug_company:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['illness:drug_company:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listDrug_company, getDrug_company, delDrug_company, addDrug_company, updateDrug_company } from "@/api/illness/drug_company";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDrug_company(this.queryParams).then(response => {
        this.drug_companyList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.drugCompanyId != null) {
            ...
            addDrug_company(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('illness/drug_company/export', {
        ...this.queryParams
      }, `drug_company_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
